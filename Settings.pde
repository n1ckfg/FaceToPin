  ...
  Settings(String _s) {
    try {
      ...
      for (int i=0;i<settings.data.length;i++) {
        ...
        if (settings.data[i].equals("oscScaleNum")) oscScaleNum = setFloat(settings.data[i+1]);
        ...
        if (settings.data[i].equals("movingFace")) movingFace = setBoolean(settings.data[i+1]);
